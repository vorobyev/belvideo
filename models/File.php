...
class File Extends ActiveRecord {
    ...
    public function save($runValidation = false, $attributeNames = NULL, $mode = NULL) {
        ...
        $probe = $ffprobe->format(Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" . $this->href.".".$ext);
        if ($probe!=null) {
        ...
        $this->createDate = date("Y-m-d H:i:s", strtotime($tags['creation_time']));
            ...
            try {
                $this->size = $this->filesize_get((int)filesize(Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" . $this->href.".".$ext));
...
//               'ffprobe.binaries' => '/home/prog/FFMPEG/ffmpeg/ffprobe',
//                ));
//        $video = $ffmpeg->open(Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" . $this->profile_pic);
        //$video->filters()->resize(new Coordinate\Dimension(300, 200))->synchronize();
...
//        $video->save(new Format\Video\WebM(), Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" .'export-webm.flv');   
        ...
        $videoInfo = "";
        $videoMeta = $ffprobe->streams(Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" . $this->href.".".$ext)->videos()->first();
        ...
        $audioInfo = "";
        $audioMeta = $ffprobe->streams(Yii::$app->params['uploadPath'] . "/" . Yii::$app->user->identity->id . "/" . $this->href.".".$ext)->audios()->first();    
        if ($audioMeta!=null) {
    ...
    public function getVideoByHref($href) {
        if ($href !== "") {
            return self::find()->where(['href' => $href])->one();
        } else{
        }
    }
