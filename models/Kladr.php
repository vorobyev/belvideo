...
class Kladr extends ActiveRecord {
    ...
    public function findAddress($value,$type,$region,$area,$city,$locality) 
    {
        if ($type=="1") {
            ...
            return self::find()->where(['and',['like','name',$value],['like','id',$region.$area.'___000%',false],['not like','id',$region.$area.'000000%',false]])->limit(10)->all();
        } elseif ($type=="4") {
            return self::find()->where(['and',['like','name',$value],['like','id',$region.$area.$city.'%',false],['not like','id',$region.$area.$city.'000%',false]])->limit(10)->all();
        }
